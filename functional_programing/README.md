...
- 현대 소프트웨어는 여러 타임라인에서 실행됩니다. 서로 다른 컴퓨터나 스레드, 프로세스, 비동기 호출과 같은 것이 있다면 새로운 타임라인을 추가합니다.
- 타임 라인 다이어그램은 코드가 순서대로 실행되는지 동시에 실행되는지를 알려줍니다.
- 자원을 공유하는 부분은 버그가 발생하기 쉽습니다. 공유자원을 확인하고 없애면 코드가 더 좋아집니다.
...
- 작업의 순서를 지키지 않으면 작업의 결과가 예측하기 힘들다. (의도하지 않는 결과 발생)
- 작업의 순서를 지키게 되면 최종 결과가 느리게 반영이 된다.
  - 속도를 높이는 방법 중 하나를 작업 큐의 사이즈를 조절해서 새로운 작업이 들어오면 큐의 맨 앞의 아이템을 버리는 방식을 사용할수 있다.

## 웹 어플리케이션에서 발생하는 동시성 프로그래밍의 복잡성
...
애플리케이션을 만들 때 사용자 인터랙션을 적게 만들어 액션을 줄일수 있다. 물론 이것은 사용자에게 좋지 않은 경험을 준다.

## 반응형 아키텍처

핵심 원칙은 이벤트에 대한 반응으로 일어날 일을 지정하는 것이다. 반응형 아키텍쳐는 웹서비스와 잘 어울린다. 웹서비스는 웹 요청 응답에 일어날 일을 지정하고, UI는 버튼 클릭과 같은 이벤트 응답에 일어날 일을 지정하면 된다.
